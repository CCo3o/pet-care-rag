...
基于 **RAG（检索增强生成）** 的宠物寄养问答系统：把养宠知识、寄养规则变成"能对话的知识库"，回答带来源引用。

> 本项目是「大模型应用开发实习」求职作品集的项目一（基础闭环版），后续会升级为：三层知识库交叉检索 → Web 界面 → 在线 Demo。
...
- [ ] Rerank 重排序，量化准确率提升
- [ ] FastAPI + Streamlit 做成 Web 应用
- [ ] Docker 打包 + 云服务器部署（在线 Demo）
- [ ] 写技术博客记录踩坑过程
